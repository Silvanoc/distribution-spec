	...
	g.Context(titlePull, func() {
		...
		g.Context("Setup", func() {
			...
			g.Specify("Populate registry with test manifest", func() {
				...
				RunOnlyIf(runPullSetup)
				tag = testTagName
				manifestRefs = pushManifestByTag(tag, manifests[0].Digest, manifests[0].Content, manifestRefs)
			})

			g.Specify("Populate registry with test manifest", func() {
				SkipIfDisabled(pull)
				RunOnlyIf(runPullSetup)
				manifestRefs = pushManifestByDigest(manifests[1].Digest, manifests[1].Content, manifestRefs)
			})
