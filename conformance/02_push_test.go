	...
	g.Context(titlePush, func() {
		...
		g.Context("Manifest Upload", func() {
			...
			g.Specify("Dangling subject [OCI-Image v1.1]", func() {
				...
				Expect(resp.Header().Get("OCI-subject")).To(MatchRegexp(`^[a-f0-9]+:[a-f0-9]+$`), "PUT manifest with 'dangling' subject does not return a header 'OCI_subject' with the subject digest")
			})
		})
